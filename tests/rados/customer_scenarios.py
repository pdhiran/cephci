...
def run(ceph_cluster, **kw):
    ...
    if config.get("BZ-1905339"):
        bz_config = config.get("BZ-1905339")
        try:
            verify_mon_db_trim(ceph_cluster=ceph_cluster, node=cephadm, **bz_config)
...
def verify_mon_db_trim(ceph_cluster, node: CephAdmin, **kwargs):
    ...
    daemon_info = run_ceph_command(node, action="daemon_info")
    ...
    status = run_ceph_command(node, action="ceph_report")
    init_commmits = {
    ...
    cluster_fsid = run_ceph_command(node, action="ceph fsid")["fsid"]
    daemon_info = run_ceph_command(node, action="daemon_info")
    osd_daemons = [entry for entry in daemon_info if entry["daemon_type"] == "osd"]
    for val in osd_nodes:
        for osd in osd_daemons:
            if re.search(osd["hostname"], val.hostname):
                ...
                val.exec_command(sudo=True, cmd=cmd)
    ...
    while end_time > datetime.datetime.now():
        status_report = run_ceph_command(node, action="ceph_report")
    ...
    if not change_config_for_slow_ops(node=node, action="rm", **kwargs):
        error = "Failed to clean-up config changes to generate slow_ops"
        raise TestCaseFailureException(error)
...
def get_slow_ops_data(node: CephAdmin, installer, action) -> bool:
    ...
    if action == "current":
        cmd = f" ceph daemon mon.{installer.hostname} ops -f json-pretty"
    ...
    # Checking all the ops reports historically
        ...
        cmd = f"ceph daemon mon.{installer.hostname} dump_historic_ops -f json-pretty"
        out, err = node.shell([cmd])
