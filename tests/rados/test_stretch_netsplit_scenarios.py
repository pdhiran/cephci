...
def run(ceph_cluster, **kw):
    ...
    set_debug = config.get("set_debug", True)
    ...
    try:
        ...
        if set_debug:
            log.debug("Setting up debug configs on the cluster for mon, osd & Mgr daemons")
            ...
            rados_obj.run_ceph_command(
                cmd="ceph config set mgr debug_mgr 20", client_exec=True
            )
        ...
        if set_debug:
            ...
            rados_obj.run_ceph_command(
                cmd="ceph config rm mgr debug_mgr", client_exec=True
            )
