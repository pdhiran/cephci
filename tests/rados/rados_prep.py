...
def run(ceph_cluster, **kw):
    ...
    if config.get("ec_pool"):
        ...
        if not create_erasure_pool(node=cephadm, name=uuid, **ec_config):
            ...
            return 1
        if not run_rados_bench(node=cephadm, pool_name=pool_name):
...
def run_rados_bench(
    node: CephAdmin, pool_name: str, operation: str = "write", **kwargs
) -> bool:
    """
    Method to trigger Write/read operations via the Rados Bench tool
    Args:
        node: Cephadm node where the commands need to be executed
        pool_name: pool on which the operation will be performed
        operation: specifies weather to perform read or write operation. Default : write
...
def create_pool(node: CephAdmin, pool_name: str, pg_num: int = 64, **kwargs) -> bool:
    ...
    cmd_map = {
        "ec_profile_name": f"{cmd} erasure",
        "min_size": f"{cmd} min_size",
        "erasure_code_use_overwrites": f"{cmd} allow_ec_overwrites",
    }
    for key in kwargs:
        if cmd_map.get(key):
            cmd = f"{cmd_map[key]} {kwargs[key]}"
    try:
        node.shell([cmd])
    except Exception as err:
    ...
    # Pool gets created with default crush rule, checking if change of crush rule is required
    if kwargs.get("crush_rule"):
        crush_cmd = (
            f"sudo ceph osd pool set {pool_name} crush_rule {kwargs.get('crush_rule')}"
        )
        node.shell([crush_cmd])

    if kwargs.get("disable_pg_autoscale"):
        cmd = f"ceph osd pool set {pool_name} pg_autoscale_mode off"
        node.shell([cmd])

    # Enabling rados application on the pool
    enable_app_cmd = f"sudo ceph osd pool application enable {pool_name} rados"
