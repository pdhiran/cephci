...
def add(cls, config: Dict) -> None:
    ...
    if nodes_:
        ...
        def setup(host):
            rhcs_version = str([host[entry].get("release", "default") for entry in host][0])
            name = [entry for entry in host][0]
            node = get_node_by_id(cls.cluster, name)

            rhel_version = node.distro_info["VERSION_ID"][0]
            log.debug(f"Passed RHCS version is : {rhcs_version} for host : {node.hostname}\n"
...
def remove(cls, config: Dict) -> None:
    ...
    if config.get("remove_admin_keyring"):
        node.exec_command(
            cmd="rm -rf /etc/ceph/ceph.client.admin.keyring", sudo=True,
        )
    ...
    if config.get("remove_packages"):
        for pkg in config.get("remove_packages"):
            node.exec_command(
                cmd=f"yum remove -y {pkg}", sudo=True,
            )
