...
def _nfs_daemon(node, cluster_id):
    """Return (container_id, hostname) for a running nfs.<cluster_id> daemon.

    Same orch-ps + ready predicate as wait_for_nfs_ganesha_daemons. Uses
    ...
    running = [d for d in daemons if not _nfs_daemon_not_ready(d)]
    if not running:
        raise OperationFailedError(
            f"No running nfs.{cluster_id} daemon (ps={daemons!r})"
        )
    daemon = running[0]
...
def list_cephfs_asoks(ceph_cluster, node, cluster_id):
    ...
    out, _ = host.exec_command(
        ...
        cmd=(
            ...
            "'ls /run/ceph/*.asok 2>/dev/null || true'"
        ),
        timeout=60,
    )
...
def verify_pec(ceph_cluster, node, cluster_id, expect_clients):
